exports.getCategory = async function(req, res) {
    try {
        ...
        extraData = await getData(businessData)
        businessList = {
            mainBusiness: businessData,
            currentUserBusiness: result[0],
            countryList: countryData,
            businessUser: userData,
            goalList: unsdgData,
            business_length: businessData.length,
            unSdg: extraData.unSdg,
            interactions: extraData.interactions,
            stakeholders: await getStakeholders(businessData),
...
async function getData(businessListData) {
    ...
    for await (let business of businessListData) {
        ...
        for await (let data of unsdgData) {
    ...
    var stakeholders_maps = new Array()
        for await (business_item of businessListData) {
            ...
            if (answersById[0] != undefined) {
                ...
                if (scoringData != null) {
                    for (stakeholderRing of scoringData) {
